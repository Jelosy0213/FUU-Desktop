...
describe('requestCenter', () => {
  it('只对可恢复的传输层错误进行重试', () => {
    ...
    expect(shouldRetryRequest(new Error('会话已失效，请重新登录'))).toBe(false)
    expect(shouldRetryRequest(new Error('验证码错误'))).toBe(false)
  })
  ...
  it('关闭重试时只执行一次请求', async () => {
    ...
    await expect(
      requestWithPolicy(
        ...
        { retry: false, timeoutMs: 100 },
      ),
    ).rejects.toThrow('网络连接失败')
    expect(attempts).toBe(1)
  })

  it('业务错误不做重试，直接抛出', async () => {
    let attempts = 0
    await expect(
      requestWithPolicy(
        '测试接口',
        ...
        { retry: true, maxAttempts: 3, retryDelayMs: 0, timeoutMs: 100 },
      ),
    ).rejects.toThrow('账号或密码错误')
    expect(attempts).toBe(1)
  })

  it('请求超时后按超时错误抛出', async () => {
    await expect(
      requestWithPolicy(
        '测试接口',
        () => new Promise((resolve) => window.setTimeout(resolve, 200)),
        { retry: false, timeoutMs: 20 },
      ),
    ).rejects.toThrow('测试接口请求超时')
  })
})
